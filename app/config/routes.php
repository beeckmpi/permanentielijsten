 ...
 */
Router::connect('/', 'Lijsten::index');
Router::connect('/download/{:filename}.{:format}', array(), function($request){
	return new Response(array(
		'headers' 	=> array('Content-type' => 'application/force-download', 'Content-Disposition', 'attachment; filename='.$request->filename.'.'.$request->format),
		'body' 		=> File::first(array('conditions' => array('filename' => $request->filename.'.'.$request->format)))->file->getBytes()
	));
});
/**
